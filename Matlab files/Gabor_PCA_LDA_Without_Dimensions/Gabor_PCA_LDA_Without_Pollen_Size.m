...
%         Zakomentirano e i programata produljava da raboti po sushtiq
%         nachin
    Features=[];
    for p = 1:8
        Features1=[];
...
%       Features2(p,:)=Features1;   Ne razbiram zashto e nujno.
%         Zakomentirano e i programata produljava da raboti po sushtiq
%         nachin
...
%     Premahvam Vliqnieto na razmera
...
PollenValues=[];
for it=1:nPatterns
    PollenValues=[PollenValues;PollenFeatures(it).GaborFeatures]; %Features fom Gabor Bank
end

% Tuk mi e malko neqsno. Ako moje obqsnenie
mapcaplot(PollenValues,TitlePollens)
[pc, zscores, pcvars] = pca(PollenValues);
Nvar=pcvars./sum(pcvars) * 100; %Tezi zashto se izchislqvat ? Ne vijdam da se izpolzvat po - natatyk
Ncum=cumsum(pcvars./sum(pcvars) * 100); %Tezi zashto se izchislqvat ? Ne vijdam da se izpolzvat po - natatyk
...
% ------------------------- LDA classifier ----------------------------- 
nTilia=size(Tilia,1); nAcacia=size(Acacia,1); nLavender=size(Lavender,1); 
nCanola=size(Canola,1); nThistle=size(Thistle,1); nLavenderMultifida=0;
B=[2.*ones(1,nTilia),1.*ones(1,nAcacia),3.*ones(1,(nLavender+nLavenderMultifida)),...
    4.*ones(1,nCanola),5.*ones(1,nThistle)];
